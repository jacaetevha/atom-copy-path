...
module.exports = {
  ...
  activate() {
    this.ignorePrefix = atom.config.get('configurable-copy-path.ignorePrefix');
    this.subscriptions = new CompositeDisposable();
    this.subscriptions.add(atom.commands.add("atom-workspace", {
      "configurable-copy-path:copy-project-relative-path-ignoring-prefix": (e) => this.copyProjectRelativePathIgnoringPrefix(e),
  ...
  getTargetEditorPath(e) {
    ...
    if (!this.ignorePrefix) {
      ...
      return item.getPath ? item.getPath().replace(this.ignorePrefix, "") : "";
    }
  },
  ...
  copyProjectRelativePathIgnoringPrefix(e) {
    const result = this
      .getProjectRelativePath(this.getTargetEditorPath(e))
      .split(this.ignorePrefix)[1];
    this.writeToClipboardIfValid(result);
  },
